...
namespace napa::voxel::testing
{
	namespace
	{
		...
		void RunSoilTransitionTests(TestContext& context) noexcept
		{
			using namespace napa::voxel;
		...
		void RunAtomicSoilMutationTests(TestContext& context) noexcept
		{
			using namespace napa::voxel;
		...
		void RunCurrentFirstAndChunkRevisionTests(TestContext& context) noexcept
		{
			using namespace napa::voxel;
			std::unique_ptr<VoxelWorld> world;
		...
		void RunExactDirtyChunkTests(TestContext& context) noexcept
		{
			using namespace napa::voxel;
		...
		void RunMutationFailureAtomicityTests(TestContext& context) noexcept
		{
			using namespace napa::voxel;
			std::unique_ptr<VoxelWorld> world;
			if (!CreateSoilWorld(world))
		...
		void RunRevisionAndAllocationFailureTests(TestContext& context) noexcept
		{
			using namespace napa::voxel;
			...
			const ValidationResult revisionResult =
				ApplySphereEditWithExhaustedRevision(
			...
			const ValidationResult successfulResult =
				ApplySphereEditWithAllocationProbe(
		...
		void RunSoilMeshReplacementTests(TestContext& context)
		{
			using namespace napa::voxel;
			std::unique_ptr<VoxelWorld> world;
			if (!CreateSoilWorld(world))
		...
		void RunFullDomainMutationOracleTests(TestContext& context) noexcept
		{
			using namespace napa::voxel;
