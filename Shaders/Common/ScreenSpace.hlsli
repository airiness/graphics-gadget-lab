...
float3 ReconstructPositionFromRawDepth(float2 uv, float rawDepth, float4x4 inverseTransform)
{
	const float2 ndc = UVToNDC(uv);
	const float4 homogeneousPosition = mul(float4(ndc, rawDepth, 1.0), inverseTransform);
	if (!all(isfinite(homogeneousPosition)) || abs(homogeneousPosition.w) <= 1.0e-8)
	{
		return 0.0.xxx;
	}
