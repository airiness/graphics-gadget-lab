...
namespace gglab
{
	...
	public:
		...
		ShaderId LoadCSO(const std::filesystem::path& path, ShaderStage stage) noexcept;
		D3D12_SHADER_BYTECODE GetBytecode(ShaderId shaderId) const noexcept;
		const ComPtr<ID3DBlob>& GetBlob(ShaderId shaderId) const noexcept;
	...
	private:
		ShaderId InsertOrGet(const ShaderKey& key, ComPtr<ID3DBlob>&& blob, ShaderStage stage) noexcept;

	private:
		static bool GetDxilValidatorHash(const void* data, size_t size, ShaderHash128& outHash) noexcept;
		static ShaderHash128 HasBlob(ID3DBlob* blob) noexcept;
