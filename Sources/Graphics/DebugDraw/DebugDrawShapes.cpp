...
namespace gglab
{
	namespace
	{
		...
		void AppendTriangle(std::vector<Vector3>& positions,
		...
		void BuildBoxGeometry(std::span<const Vector3, 8> corners,
			DebugDrawFillMode fillMode, std::vector<Vector3>& positions)
		{
	...
	void DebugDrawContext::Box(const Vector3& center, const Vector3& extents,
		const DebugDrawStyle& style) noexcept
	{
		...
		m_System->Submit(
			style.m_FillMode == DebugDrawFillMode::Wireframe ?
				DebugDrawSystem::PrimitiveTopology::Lines : DebugDrawSystem::PrimitiveTopology::Triangles,
			positions, style);
	}

	void DebugDrawContext::Obb(const Matrix& transform, const Vector3& extents,
		const DebugDrawStyle& style) noexcept
	{
		if (!m_System || !math::IsFinite(extents) ||
			extents.m_X < 0.0f || extents.m_Y < 0.0f || extents.m_Z < 0.0f)
	...
	void DebugDrawContext::Circle(const Vector3& center, const Vector3& normal,
		float radius, const DebugDrawStyle& style, uint32_t segments) noexcept
	{
		...
		if (style.m_FillMode == DebugDrawFillMode::Wireframe)
		{
			...
			for (uint32_t segment = 0; segment < segments; ++segment)
			{
				...
				AppendLine(positions,
					CirclePoint(center, basis, radius, a0),
		...
		for (uint32_t segment = 0; segment < segments; ++segment)
		{
			const float a0 = math::TwoPi * segment / segments;
			const float a1 = math::TwoPi * (segment + 1) / segments;
			AppendTriangle(positions, center,
				CirclePoint(center, basis, radius, a0),
				CirclePoint(center, basis, radius, a1));
		}
	...
	void DebugDrawContext::Sphere(const Vector3& center, float radius,
		const DebugDrawStyle& style, uint32_t segments) noexcept
	{
		...
		if (style.m_FillMode == DebugDrawFillMode::Wireframe)
		{
			for (uint32_t latitude = 1; latitude < latitudeSegments; ++latitude)
			{
				...
				{
					AppendLine(positions, point(latitude, longitude), point(latitude, longitude + 1));
				}
			}
			for (uint32_t longitude = 0; longitude < segments; ++longitude)
			{
				...
				{
					AppendLine(positions, point(latitude, longitude), point(latitude + 1, longitude));
				}
			}
			m_System->Submit(DebugDrawSystem::PrimitiveTopology::Lines, positions, style);
			return;
	...
	void DebugDrawContext::Cone(const Vector3& apex, const Vector3& direction,
		float height, float radius, const DebugDrawStyle& style, uint32_t segments) noexcept
	{
		Basis basis{};
		if (!m_System || !BuildBasis(direction, basis) ||
			!std::isfinite(height) || !std::isfinite(radius) || height <= 0.0f || radius <= 0.0f)
		{
			if (m_System) m_System->RejectInvalid();
			return;
		}
		...
		for (uint32_t segment = 0; segment < segments; ++segment)
		{
			const Vector3 p0 = CirclePoint(baseCenter, basis, radius, math::TwoPi * segment / segments);
			const Vector3 p1 = CirclePoint(baseCenter, basis, radius, math::TwoPi * (segment + 1) / segments);
			if (style.m_FillMode == DebugDrawFillMode::Wireframe)
			{
	...
	void DebugDrawContext::Cylinder(const Vector3& center, const Vector3& axis,
		float height, float radius, const DebugDrawStyle& style, uint32_t segments) noexcept
	{
		Basis basis{};
	...
	void DebugDrawContext::Capsule(const Vector3& center, const Vector3& axis,
		float halfHeight, float radius, const DebugDrawStyle& style, uint32_t segments) noexcept
	{
		Basis basis{};
		if (!m_System || !BuildBasis(axis, basis) || !std::isfinite(halfHeight) ||
			!std::isfinite(radius) || halfHeight < 0.0f || radius <= 0.0f)
		{
			if (m_System) m_System->RejectInvalid();
			return;
		}
		...
		struct Ring { float m_Axial; float m_Radius; };
	...
	void DebugDrawContext::Frustum(std::span<const Vector3, 8> corners,
		const DebugDrawStyle& style) noexcept
	{
		if (!m_System)
		{
