...
namespace gglab
{
	...
	private:
		...
		TextureUploadData MakeTextureUploadData(TextureID textureId, TextureAssetData&& textureData,
			const TextureImportSettings& importSettings,
			AssetContentFingerprint contentFingerprint = {}) noexcept;
		TextureUploadData MakeTextureUploadData(TextureID textureId, TextureArtifactHandle artifact,
			const TextureImportSettings& importSettings,
			AssetContentFingerprint contentFingerprint = {}, SourceDigest sourceDigest = {},
			DerivedDataKey derivedDataKey = {}) noexcept;
		[[nodiscard]] bool UploadTexture(const TextureUploadData& uploadData,
		...
		[[nodiscard]] TaskHandle RequestResidency(TextureID textureId, uint64_t generation,
		...
		void CreateTextureEntry(TextureID id, std::string_view textureName,
			TextureImportSettings importSettings,
			const std::filesystem::path& sourcePath = {}) noexcept;
		void CompleteTextureUpload(TextureID textureId, bool succeeded,
			AssetResidencyOperation residencyOperation = {}) noexcept;
		bool QueueTextureUpload(TextureUploadData&& uploadData,
			TaskPriority priority = TaskPriority::Normal,
			AssetResidencyOperation residencyOperation = {}) noexcept;
		bool RecordTextureUpload(TextureID textureId, uint64_t generation, TaskPriority priority,
			TextureUploadData uploadData, AssetResidencyOperation residencyOperation = {}) noexcept;
		...
		void CompleteTextureLoad(AssetOperationToken operation, TextureSemantic semantic,
			const TaskCompletionInfo& completion, TextureArtifactHandle artifact,
			AssetContentFingerprint contentFingerprint, SourceDigest sourceDigest,
			DerivedDataKey derivedDataKey, bool residencyReload,
			AssetResidencyOperation residencyOperation = {}) noexcept;
		...
		[[nodiscard]] TaskHandle QueueTextureLoad(TextureID textureId,
			const std::filesystem::path& canonicalPath, const TextureImportSettings& importSettings,
			TaskPriority priority, bool residencyReload = false,
			AssetResidencyOperation residencyOperation = {}) noexcept;
		bool RemoveTexture(TextureID textureId) noexcept;
		void SetTextureState(Texture& texture, AssetState state,
