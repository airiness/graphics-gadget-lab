...
namespace gglab
{
	namespace
	{
		...
		[[nodiscard]] bool HasHigherPriority(TaskPriority lhs, TaskPriority rhs) noexcept
		{
			return static_cast<uint8_t>(lhs) < static_cast<uint8_t>(rhs);
		}

		void RecordExecutionSample(std::deque<double>& samples, double milliseconds) noexcept
		{
		...
		[[nodiscard]] double ExecutionP95(const std::deque<double>& samples)
		{
			if (samples.empty())
			{
				return 0.0;
		...
		AssetStreamingWorkDesc desc, std::unique_ptr<IResourcePublicationJob>&& job) noexcept
	{
		GGLAB_ASSERT_MSG(IsOwnerThread(),
			"Asset resource publication must be enqueued on the scheduler owner thread.");
		...
		publication.m_Payload.m_RemainingSourceBytes = publication.m_Desc.m_Estimate.m_SourceBytes;
		...
		m_ResourcePublicationTelemetry.m_HighWatermark =
			std::max(m_ResourcePublicationTelemetry.m_HighWatermark,
	...
	uint32_t AssetUploadScheduler::CancelReadyWork(const AssetStreamingIdentity& identity) noexcept
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "Asset ready work cancellation must run on the owner thread.");
		if (!IsOwnerThread())
		{
			return 0;
		}
		...
		return CancelQueuedWork(m_CpuPayloadQueue, m_CpuPayloadTelemetry, identity, false) +
			...
			CancelQueuedWork(m_UploadRecordingQueue, m_UploadRecordingTelemetry, identity, true);
	}
	...
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "Asset streaming work reprioritization must run on the owner thread.");
			...
			std::ranges::stable_sort(m_GpuFinalizeQueue,
				[](const PendingGpuFinalize& lhs, const PendingGpuFinalize& rhs) noexcept
				{
					return HasHigherPriority(
						lhs.m_Upload.m_Desc.m_Priority, rhs.m_Upload.m_Desc.m_Priority);
				});
		}
		return updatedCount;
	}
	...
	AssetUploadHandle AssetUploadScheduler::RecordUpload(
		AssetUploadDesc desc, AssetUploadRecord record, AssetUploadCompletion completion) noexcept
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "Asset uploads must be recorded on the scheduler owner thread.");
		GGLAB_ASSERT_MSG(m_IsRecordingUploadBatch,
		...
		if (!m_RecordingBatch)
		{
			m_RecordingBatch = std::make_unique<TransferBatch>(m_TransferManager->BeginBatch());
		}
		...
		catch (const std::exception& exception)
		{
			...
			GGLAB_LOG_GRAPHICS_ERROR("Asset upload recording '{}' threw an exception: {}",
				upload.m_Desc.m_Name, exception.what());
		}
		catch (...)
		{
			GGLAB_LOG_GRAPHICS_ERROR(
				"Asset upload recording '{}' threw an unknown exception.", upload.m_Desc.m_Name);
		}
	...
	void AssetUploadScheduler::FlushRecordedUploads() noexcept
	{
		...
		{
			GGLAB_ASSERT_MSG(!m_RecordingBatch && m_RecordedUploadBytes == 0,
		...
		for (PendingUpload& recorded : m_RecordedUploads)
		{
			...
			{
				ProgressReporter(recorded.m_Desc.m_Progress)
					.Report(0.72f, "GPU upload submission failed", recorded.m_Desc.m_Name);
				EnqueueGpuFinalize(std::move(recorded), AssetUploadStatus::Failed);
				continue;
			}

			ProgressReporter(recorded.m_Desc.m_Progress)
				.Report(0.82f, "Waiting for GPU upload",
	...
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "Asset upload fences must be polled on the scheduler owner thread.");
		if (!IsOwnerThread())
		{
			return 0;
		}
	...
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "AssetUploadScheduler::DrainReadyWork must run on the owner thread.");
		if (!IsOwnerThread())
		{
			return;
		}
	...
	uint32_t AssetUploadScheduler::PollCompletedUploads() noexcept
	{
		...
		for (PendingUpload& upload : completed)
		{
			...
			GGLAB_ASSERT_MSG(stagingBytes <= m_InFlightBytes,
				"Asset upload in-flight byte accounting underflow.");
			m_InFlightBytes = stagingBytes <= m_InFlightBytes ? m_InFlightBytes - stagingBytes : 0;
			const AssetUploadStatus status = upload.m_RecordingSucceeded
				? AssetUploadStatus::Succeeded
	...
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "AssetUploadScheduler::Finalize must run on the owner thread.");
		if (!IsOwnerThread())
		{
			return;
		}
		...
		GGLAB_UNUSED(PollCompletedUploads());
		GGLAB_UNUSED(DrainGpuFinalizeQueue(true));
		GGLAB_ASSERT_MSG(m_PendingUploads.empty(),
	...
	void AssetUploadScheduler::SetFrameBudget(const AssetStreamingFrameBudget& budget) noexcept
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "Asset streaming frame budget must be changed on the owner thread.");
		if (IsOwnerThread())
		{
	...
	{
		GGLAB_ASSERT_MSG(IsOwnerThread(),
			"Resource publication fault injection must be configured on the owner thread.");
		if (!IsOwnerThread())
		{
			return;
		}
		GGLAB_ASSERT_MSG(
			fault.IsValid(), "Invalid resource publication fault injection configuration.");
	...
	{
		GGLAB_ASSERT_MSG(IsOwnerThread(),
			"Resource publication fault injection must be cleared on the owner thread.");
		if (IsOwnerThread())
		{
			m_PublicationFault = {};
			m_PublicationFaultObservedOccurrences = 0;
	...
	void AssetUploadScheduler::ArmGpuCompletionHold(const AssetStreamingIdentity& identity) noexcept
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "GPU completion hold must be configured on the owner thread.");
		GGLAB_ASSERT_MSG(identity.m_Kind != AssetStreamingWorkKind::Unknown,
	...
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "GPU completion hold must be cleared on the owner thread.");
		if (IsOwnerThread())
		{
			m_GpuCompletionHold = {};
		}
	...
	{
		GGLAB_ASSERT_MSG(
			IsOwnerThread(), "AssetUploadScheduler statistics must be read on the owner thread.");
		AssetUploadStatistics statistics{};
		statistics.m_CpuPayloadQueue =
			BuildQueueStatistics(m_CpuPayloadQueue, m_CpuPayloadTelemetry);
		statistics.m_ResourcePublicationQueue = BuildResourcePublicationQueueStatistics();
		statistics.m_UploadRecordingQueue =
			BuildQueueStatistics(m_UploadRecordingQueue, m_UploadRecordingTelemetry);
		...
		statistics.m_UploadPromotionBudgetDeferralCount = m_UploadPromotionBudgetDeferralCount;
		...
		for (const PendingUpload& upload : m_PendingUploads)
		{
			const ProgressSnapshot progress = upload.m_Desc.m_Progress
				? upload.m_Desc.m_Progress->GetSnapshot()
				: ProgressSnapshot{};
		...
		std::deque<QueuedWork>& queue, QueueTelemetry& telemetry, QueuedWork&& queued) noexcept
	{
		...
		telemetry.m_HighWatermark =
			std::max(telemetry.m_HighWatermark, static_cast<uint32_t>(queue.size()));
		...
		QueuedWork&& queued, QueueTelemetry& telemetry, std::string_view queueName) noexcept
	{
		const double queueMilliseconds =
			Milliseconds(std::chrono::steady_clock::now() - queued.m_QueuedAt);
		telemetry.m_TotalQueueMilliseconds += queueMilliseconds;
		++telemetry.m_QueueSampleCount;
		telemetry.m_MaxQueueMilliseconds =
			std::max(telemetry.m_MaxQueueMilliseconds, queueMilliseconds);
		...
		catch (const std::exception& exception)
		{
			...
			GGLAB_LOG_GRAPHICS_ERROR("Asset {} work '{}' threw an exception: {}", queueName,
				queued.m_Desc.m_Name, exception.what());
		}
		catch (...)
		{
			++telemetry.m_CallbackFailureCount;
			GGLAB_LOG_GRAPHICS_ERROR(
				"Asset {} work '{}' threw an unknown exception.", queueName, queued.m_Desc.m_Name);
		}
		const double executionMilliseconds =
			Milliseconds(std::chrono::steady_clock::now() - executionBegin);
		telemetry.m_TotalExecutionMilliseconds += executionMilliseconds;
		telemetry.m_MaxExecutionMilliseconds =
			std::max(telemetry.m_MaxExecutionMilliseconds, executionMilliseconds);
		RecordExecutionSample(telemetry.m_RecentExecutionMilliseconds, executionMilliseconds);
		...
		while (processedCount < initialCount && !m_CpuPayloadQueue.empty())
		{
			...
			if (!ignoreBudget && processedCount > 0 &&
				(m_LastFrameUsage.m_CpuPayloadItems >= m_FrameBudget.m_MaxCpuPayloadItems ||
					m_LastFrameUsage.m_CpuPayloadMilliseconds >=
					m_FrameBudget.m_MaxCpuPayloadMilliseconds))
			{
				break;
			}
			...
			const double elapsed =
				ExecuteWork(std::move(queued), m_CpuPayloadTelemetry, "CPU-payload");
			++processedCount;
	...
	bool AssetUploadScheduler::TryApplyResourcePublicationFault(
		const AssetStreamingIdentity& identity, AssetResourcePublicationStage stage,
		AssetResourcePublicationFaultTiming timing,
		AssetResourcePublicationStepResult& result) noexcept
	{
		if (!m_PublicationFault.IsValid() || m_PublicationFault.m_Identity != identity ||
			m_PublicationFault.m_Stage != stage || m_PublicationFault.m_Timing != timing)
		{
			return false;
		}

		++m_PublicationFaultObservedOccurrences;
		if (m_PublicationFaultObservedOccurrences < m_PublicationFault.m_TriggerOccurrence)
		{
			return false;
		}

		const AssetResourcePublicationFaultAction action = m_PublicationFault.m_Action;
		...
		result.m_Error = std::format("Injected publication {} {} stage {} occurrence {}",
			...
			static_cast<size_t>(stage), m_PublicationFaultObservedOccurrences);
		...
		while (!m_ResourcePublicationQueue.empty())
		{
			...
			if (ignoreBudget && processedCount >= MaxResourcePublicationDrainSteps)
			{
				GGLAB_LOG_GRAPHICS_ERROR(
					...
					MaxResourcePublicationDrainSteps, m_ResourcePublicationQueue.size());
				while (!m_ResourcePublicationQueue.empty())
				{
					QueuedResourcePublication publication =
					...
					publication.m_Job->Abort(
						context, AssetResourcePublicationAbortReason::Shutdown);
			...
			QueuedResourcePublication publication = std::move(m_ResourcePublicationQueue.front());
			m_ResourcePublicationQueue.pop_front();
			context.m_Priority = publication.m_Desc.m_Priority;
			if (!publication.m_HasStarted)
			{
				const double queueMilliseconds =
					Milliseconds(std::chrono::steady_clock::now() - publication.m_QueuedAt);
				...
				m_ResourcePublicationTelemetry.m_MaxQueueMilliseconds = std::max(
					m_ResourcePublicationTelemetry.m_MaxQueueMilliseconds, queueMilliseconds);
			...
			const bool injectedBeforeStep = TryApplyResourcePublicationFault(
				publication.m_Desc.m_Identity, publication.m_Job->GetCurrentStage(),
				AssetResourcePublicationFaultTiming::BeforeStep, result);
			...
			m_ResourcePublicationTelemetry.m_MaxExecutionMilliseconds = std::max(
				m_ResourcePublicationTelemetry.m_MaxExecutionMilliseconds, executionMilliseconds);
			RecordExecutionSample(m_ResourcePublicationTelemetry.m_RecentExecutionMilliseconds,
			...
			{
				PublicationStageTelemetry& stageTelemetry = m_PublicationStageTelemetry[stageIndex];
				++stageTelemetry.m_StepCount;
				stageTelemetry.m_TotalMilliseconds += executionMilliseconds;
				stageTelemetry.m_MaxMilliseconds =
					std::max(stageTelemetry.m_MaxMilliseconds, executionMilliseconds);
				RecordExecutionSample(
					stageTelemetry.m_RecentExecutionMilliseconds, executionMilliseconds);
			}
			if (result.m_Status == AssetResourcePublicationStepStatus::Continue)
			{
				GGLAB_UNUSED(TryApplyResourcePublicationFault(publication.m_Desc.m_Identity,
					result.m_Usage.m_Stage, AssetResourcePublicationFaultTiming::AfterStep,
					result));
			}
			...
			case AssetResourcePublicationStepStatus::Failed:
				...
				publication.m_Job->Abort(context, AssetResourcePublicationAbortReason::Failed);
				GGLAB_LOG_GRAPHICS_ERROR("Resource publication '{}' failed: {}",
			...
			case AssetResourcePublicationStepStatus::Cancelled:
				++m_ResourcePublicationTelemetry.m_CancelledCount;
				publication.m_Job->Abort(context, AssetResourcePublicationAbortReason::Cancelled);
				break;
			}
	...
	{
		GGLAB_ASSERT_MSG(!m_IsRecordingUploadBatch && !m_RecordingBatch &&
		...
		while (processedCount < initialCount && !m_UploadRecordingQueue.empty())
		{
			...
			const bool exceedsFrameBudget =
				...
				m_FrameBudget.m_MaxUploadRecordingItems ||
				m_LastFrameUsage.m_UploadBytes + estimate.m_StagingBytes >
				m_FrameBudget.m_MaxUploadBytes ||
				m_LastFrameUsage.m_UploadOperations + estimate.m_OperationCount >
				m_FrameBudget.m_MaxUploadOperations ||
				m_LastFrameUsage.m_UploadRecordingMilliseconds >=
				m_FrameBudget.m_MaxUploadRecordingMilliseconds;
			...
			if (!ignoreBudget && !hasFrameAdmission &&
				(exceedsFrameBudget || exceedsInFlightBudget))
			{
			...
			const double elapsed =
				ExecuteWork(std::move(queued), m_UploadRecordingTelemetry, "upload-recording");
			++processedCount;
			++m_LastFrameUsage.m_UploadRecordingItems;
	...
	void AssetUploadScheduler::EnqueueGpuFinalize(
		PendingUpload&& upload, AssetUploadStatus status) noexcept
	{
		...
		m_GpuFinalizeTelemetry.m_HighWatermark = std::max(m_GpuFinalizeTelemetry.m_HighWatermark,
		...
		while (processedCount < initialCount && !m_GpuFinalizeQueue.empty())
		{
			if (!ignoreBudget && processedCount > 0 &&
				(m_LastFrameUsage.m_GpuFinalizeItems >= m_FrameBudget.m_MaxGpuFinalizeItems ||
					m_LastFrameUsage.m_GpuFinalizeMilliseconds >=
					m_FrameBudget.m_MaxGpuFinalizeMilliseconds))
			{
				break;
			}
			...
			m_GpuFinalizeTelemetry.m_MaxQueueMilliseconds =
				std::max(m_GpuFinalizeTelemetry.m_MaxQueueMilliseconds, queueMilliseconds);
			const auto executionBegin = std::chrono::steady_clock::now();
			...
			m_GpuFinalizeTelemetry.m_MaxExecutionMilliseconds =
				std::max(m_GpuFinalizeTelemetry.m_MaxExecutionMilliseconds, executionMilliseconds);
			++processedCount;
	...
	{
		GGLAB_ASSERT_MSG(estimate.m_SourceBytes <= m_ReadyPayloadBytes,
			"Asset ready payload byte accounting underflow.");
		m_ReadyPayloadBytes = estimate.m_SourceBytes <= m_ReadyPayloadBytes
			? m_ReadyPayloadBytes - estimate.m_SourceBytes
			: 0;
		if (uploadRecording)
		{
			GGLAB_ASSERT_MSG(estimate.m_SourceBytes <= m_UploadRecordingBacklogBytes,
				"Asset upload-recording payload byte accounting underflow.");
			m_UploadRecordingBacklogBytes =
				estimate.m_SourceBytes <= m_UploadRecordingBacklogBytes
				? m_UploadRecordingBacklogBytes - estimate.m_SourceBytes
				: 0;
		}
	}
		...
		const AssetResourcePublicationStepUsage& usage) noexcept
	{
		...
		publication.m_Desc.m_Estimate.m_SourceBytes = publication.m_Payload.m_RemainingSourceBytes;
	...
	{
		GGLAB_ASSERT_MSG(bytes <= m_ReadyPayloadBytes,
			"Resource publication ready payload accounting underflow.");
		m_ReadyPayloadBytes = bytes <= m_ReadyPayloadBytes ? m_ReadyPayloadBytes - bytes : 0;
	}

	uint32_t AssetUploadScheduler::CancelQueuedWork(std::deque<QueuedWork>& queue,
		QueueTelemetry& telemetry, const AssetStreamingIdentity& identity,
	...
	uint32_t AssetUploadScheduler::UpdateQueuedWorkPriority(std::deque<QueuedWork>& queue,
		const AssetStreamingIdentity& identity, TaskPriority priority) noexcept
	{
		...
		if (updatedCount > 0)
		{
			std::ranges::stable_sort(queue,
				...
				{ return HasHigherPriority(lhs.m_Desc.m_Priority, rhs.m_Desc.m_Priority); });
		}
		return updatedCount;
	}
		...
		const std::deque<QueuedWork>& queue, const QueueTelemetry& telemetry) const
	{
		...
		statistics.m_ExecutionP95Milliseconds =
			ExecutionP95(telemetry.m_RecentExecutionMilliseconds);
		...
		for (const QueuedWork& work : queue)
		{
			...
			statistics.m_PendingWork.push_back({
				...
				.m_Progress = work.m_Desc.m_Progress ? work.m_Desc.m_Progress->GetSnapshot()
													 : ProgressSnapshot{},
				});
		}
		return statistics;
	...
	AssetStreamingQueueStatistics AssetUploadScheduler::BuildResourcePublicationQueueStatistics()
		const
	{
		GGLAB_ASSERT_MSG(m_ResourcePublicationTelemetry.m_EnqueuedCount ==
			m_ResourcePublicationTelemetry.m_CompletedCount +
			m_ResourcePublicationTelemetry.m_FailedCount +
			m_ResourcePublicationTelemetry.m_CancelledCount +
		...
		statistics.m_SourceBytesReleased = m_ResourcePublicationTelemetry.m_SourceBytesReleased;
		...
		statistics.m_MaxQueueMilliseconds = m_ResourcePublicationTelemetry.m_MaxQueueMilliseconds;
		...
		statistics.m_ExecutionP95Milliseconds =
			ExecutionP95(m_ResourcePublicationTelemetry.m_RecentExecutionMilliseconds);
		statistics.m_OverBudgetExecutionCount = m_PublicationOverBudgetExecutionCount;
		statistics.m_NoProgressContinueCount = m_PublicationNoProgressContinueCount;
		statistics.m_FaultInjectionCount = m_PublicationFaultInjectionCount;
		for (size_t stageIndex = 0; stageIndex < m_PublicationStageTelemetry.size(); ++stageIndex)
		{
			const PublicationStageTelemetry& source = m_PublicationStageTelemetry[stageIndex];
			...
			destination.m_P95Milliseconds = ExecutionP95(source.m_RecentExecutionMilliseconds);
		}
		...
		{
			statistics.m_PendingSourceBytes += publication.m_Payload.m_RemainingSourceBytes;
			statistics.m_PendingWork.push_back({
				...
				.m_Progress = publication.m_Desc.m_Progress
								  ? publication.m_Desc.m_Progress->GetSnapshot()
								  : ProgressSnapshot{},
				});
		}
		return statistics;
	...
	AssetStreamingQueueStatistics AssetUploadScheduler::BuildGpuFinalizeQueueStatistics() const
	{
		...
		statistics.m_TotalExecutionMilliseconds =
			m_GpuFinalizeTelemetry.m_TotalExecutionMilliseconds;
		...
		for (const PendingGpuFinalize& publication : m_GpuFinalizeQueue)
		{
			...
			statistics.m_PendingStagingBytes +=
				publication.m_Upload.m_Desc.m_Estimate.m_StagingBytes;
			statistics.m_PendingOperationCount +=
				publication.m_Upload.m_Desc.m_Estimate.m_OperationCount;
			statistics.m_PendingWork.push_back({
				...
				.m_Progress = publication.m_Upload.m_Desc.m_Progress
								  ? publication.m_Upload.m_Desc.m_Progress->GetSnapshot()
								  : ProgressSnapshot{},
				});
		}
		return statistics;
	...
	void AssetUploadScheduler::FinishUpload(
		PendingUpload&& upload, AssetUploadStatus status) noexcept
	{
		ProgressReporter(upload.m_Desc.m_Progress)
			.Report(status == AssetUploadStatus::Succeeded ? 0.96f : 0.82f,
				status == AssetUploadStatus::Succeeded ? "Publishing GPU resource"
				: "GPU upload failed",
		...
		info.m_ElapsedMilliseconds =
			Milliseconds(std::chrono::steady_clock::now() - upload.m_SubmittedAt);
		...
		if (upload.m_Completion)
		{
			...
			catch (const std::exception& exception)
			{
				...
				GGLAB_LOG_GRAPHICS_ERROR("Asset upload completion '{}' threw an exception: {}",
					info.m_Name, exception.what());
			}
			catch (...)
			{
				++m_CompletionCallbackFailureCount;
				GGLAB_LOG_GRAPHICS_ERROR(
					"Asset upload completion '{}' threw an unknown exception.", info.m_Name);
			}
		}
