...
namespace gglab
{
	DX12CommandQueue::DX12CommandQueue(const CreateInfo& createInfo) noexcept :
		m_DX12Device(createInfo.m_DX12Device), m_Fence(std::make_unique<DX12Fence>(m_DX12Device))
	{
		GGLAB_ASSERT(m_DX12Device);

		D3D12_COMMAND_QUEUE_DESC desc = { .Type = createInfo.m_Type,
			.Priority = createInfo.m_Priority,
			.Flags = createInfo.m_Flags,
			.NodeMask = 0 };

		GGLAB_HR_DX(
			m_DX12Device->Get()->CreateCommandQueue(&desc, IID_PPV_ARGS(&m_D3D12CommandQueue)),
			m_DX12Device->Get());
	}

	DX12CommandQueue::~DX12CommandQueue() = default;

	DX12FencePoint DX12CommandQueue::Execute(
		std::span<const DX12CommandList* const> commandLists) noexcept
	{
		...
		m_D3D12CommandQueue->ExecuteCommandLists(
			static_cast<UINT>(d3d12CommandLists.size()), d3d12CommandLists.data());

		return Signal();
	}

	RHIFencePoint DX12CommandQueue::Submit(
		std::span<const DX12CommandList* const> commandLists) noexcept
	{
