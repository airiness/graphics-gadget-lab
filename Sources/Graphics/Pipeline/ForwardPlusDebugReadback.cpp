...
namespace gglab
{
	...
	{
		if (!m_Device || bufferIndex >= m_Buffers.size() || bufferIndex >= m_PendingSlots.size())
		{
			return;
		}
		...
		std::memcpy(std::addressof(result.m_Header),
			static_cast<const std::byte*>(mapped) + HeaderReadbackOffset, sizeof(result.m_Header));
		...
		if (!m_LoggedFirstResult)
		{
			GGLAB_LOG_GRAPHICS_INFO(
				...
				result.m_FrameSerial, result.m_TileX, result.m_TileY, result.m_Header.m_Offset,
