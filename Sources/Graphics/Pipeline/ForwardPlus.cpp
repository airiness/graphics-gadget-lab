...
namespace gglab
{
	ForwardPlusTileGrid MakeForwardPlusTileGrid(
		uint32_t width,
		uint32_t height) noexcept
	{
		...
		const uint32_t tileCountX =
			(width + ForwardPlusTileSize - 1) /
				ForwardPlusTileSize;
		const uint32_t tileCountY =
			(height + ForwardPlusTileSize - 1) /
				ForwardPlusTileSize;
		const uint64_t tileCount =
			static_cast<uint64_t>(tileCountX) *
			static_cast<uint64_t>(tileCountY);
		if (tileCount >
	...
	{
		if ((simulatedWaveSize != 32 &&
				simulatedWaveSize != 64) ||
			lightHits.size() >
		...
		std::vector<uint32_t> waveCounts(
			(lightHits.size() + simulatedWaveSize - 1) /
				simulatedWaveSize);
		for (size_t lightIndex = 0;
			lightIndex < lightHits.size();
			++lightIndex)
		{
			if (lightHits[lightIndex] != 0)
			{
				++waveCounts[
					lightIndex / simulatedWaveSize];
			}
		}
		...
		for (size_t waveIndex = 0;
			waveIndex < waveCounts.size();
			++waveIndex)
		{
		...
		std::vector<uint32_t> waveLocalOffsets(
			waveCounts.size());
		for (size_t lightIndex = 0;
			lightIndex < lightHits.size();
			++lightIndex)
		{
			...
			const size_t waveIndex =
				lightIndex / simulatedWaveSize;
			const uint32_t outputIndex =
				waveOffsets[waveIndex] +
				waveLocalOffsets[waveIndex]++;
			result[outputIndex] =
