...
namespace gglab
{
	namespace
	{
		...
		const LabParameterId FixtureId(
			"forward_plus.fixture");
		const LabParameterId SelectedTileModeId(
			"forward_plus.selected_tile");
		const LabParameterId EnableCameraInputId(
			"forward_plus.camera.enable_input");
	...
	ForwardPlusLabSession::ForwardPlusLabSession(
	...
	{
		GetMutableViewRenderProfile().
			m_Lighting.m_ForwardPlus.m_Enabled = true;
		...
		GGLAB_UNUSED(parameters.Add({
			...
			.m_Impact =
				LabChangeImpact::RebuildScene,
			.m_DefaultValue = int32_t(
				ForwardPlusFixture::
					SixtyFourLocalLights),
			.m_EnumItems = {
				{
	...
	void ForwardPlusLabSession::BeginPrepare() noexcept
	{
		...
		BuildScene();
		m_LoadingProgress =
			m_AssetPreparation.BuildProgress(
				*m_Services.m_AssetManager,
				"Preparing Forward+ Lab");
	}
	...
	void ForwardPlusLabSession::Update(
	...
	void ForwardPlusLabSession::OnResize(
		uint32_t width,
	...
	void ForwardPlusLabSession::
		ApplyImmediateParameters() noexcept
	{
		m_EnableCameraInput =
			GetParameters().Get(
				EnableCameraInputId,
	...
	void ForwardPlusLabSession::
		OnParametersRestoredForPrepare(
	...
	void ForwardPlusLabSession::BuildScene() noexcept
	{
		...
		wallTransform.m_Position =
			Vector3(0.0f, 1.0f, 9.0f);
		wallTransform.m_Scale =
			Vector3(7.5f, 4.2f, 0.35f);
		const entt::entity wall =
			primitive::Cube::Create({
				.m_AssetManager =
					m_Services.m_AssetManager,
				.m_SamplerRegistry =
					m_Services.m_Renderer->
						GetSamplerRegistry(),
				.m_World = &m_World,
				.m_Transform = wallTransform,
				.m_MaterialInstance =
					MakeMaterial(
						"gglab.lab.forward_plus.wall",
						Color(
							0.11f,
							0.13f,
							0.18f,
		...
		sphereTransform.m_Position =
			Vector3(0.0f, 0.5f, 6.5f);
		sphereTransform.m_Scale =
			Vector3::One * 1.4f;
		const entt::entity sphere =
			primitive::Sphere::Create({
				.m_AssetManager =
					m_Services.m_AssetManager,
				.m_SamplerRegistry =
					m_Services.m_Renderer->
	...
	void ForwardPlusLabSession::BuildLighting() noexcept
	{
		auto& registry =
			m_World.GetRegistry();
		const auto fixture =
			static_cast<ForwardPlusFixture>(
				GetParameters().Get(
					FixtureId,
					int32_t(
						ForwardPlusFixture::
			...
			++lightIndex)
		{
			...
			if (fixture ==
					ForwardPlusFixture::NearPlaneLight ||
				(fixture ==
						ForwardPlusFixture::
							SixtyFourLocalLights &&
					lightIndex == 0))
			{
				...
				ForwardPlusFixture::TileBoundaryLight)
			{
				transform.m_Position =
					Vector3(0.0f, 0.5f, 6.5f);
			}
	...
	void ForwardPlusLabSession::
		ApplyCameraPreset() noexcept
	{
		GetCamera().LookAt(
			Vector3(0.0f, 1.0f, -7.0f),
			Vector3(0.0f, 0.7f, 7.0f));
		GetCamera().SetFov(52.0f);
		GetCamera().SetNearFar(
			0.1f,
			250.0f);
		GetCamera().Update();
	}
		...
		UpdateSelectedTile() noexcept
	{
		...
		if (mode ==
			SelectedTileMode::
				BackgroundTopLeft)
		{
			m_DebugReadback->SetSelectedTile(
				0,
				0);
			return;
		}
		...
		m_DebugReadback->SetSelectedTile(
			tileGrid.IsValid() ?
				tileGrid.m_TileCountX / 2 :
				0,
			tileGrid.IsValid() ?
				tileGrid.m_TileCountY / 2 :
				0);
	}

	void ForwardPlusLabSession::BuildDiagnostics(
		LabDiagnosticsSnapshot& diagnostics) const noexcept
	{
		diagnostics.m_Title =
			"Fixed-stride Forward+ Cull";
		if (!m_DebugReadback)
		{
			return;
		}

		const ForwardPlusTileReadback result =
			m_DebugReadback->GetLatest();
		const auto fixture =
			static_cast<ForwardPlusFixture>(
				GetParameters().Get(
					FixtureId,
		...
		const RHIDevice* device =
			m_Services.m_Renderer ?
				m_Services.m_Renderer->GetDevice() :
				nullptr;
		const RHIShaderWaveCapabilities
			waveCapabilities =
				device ?
					device->
						GetShaderWaveCapabilities() :
					RHIShaderWaveCapabilities{};
		const std::string waveLaneRange =
			waveCapabilities.IsValid() ?
				std::format(
					"{} - {}",
					waveCapabilities.m_MinLaneCount,
					waveCapabilities.m_MaxLaneCount) :
				"not reported";
		const uint32_t tileIndex =
			result.m_IsValid ?
				result.m_TileY *
					result.m_TileGrid.m_TileCountX +
					result.m_TileX :
				0;
		...
		{
			sortedIndices &=
				result.m_LightIndices[index] <
					MaxLightCapacity;
			if (index > 0)
			{
				sortedIndices &=
					result.m_LightIndices[index - 1] <
					result.m_LightIndices[index];
			}
		}
		...
		{
			indexPreview.clear();
			const uint32_t previewCount =
				std::min(lightCount, 12u);
			for (uint32_t index = 0;
				index < previewCount;
				++index)
			{
		...
		diagnostics.m_Metrics = {
			{
				...
				.m_Value = device ?
					std::string(
						device->
							GetAdapterCompatibilityIdentity()) :
					"unavailable",
			},
			{
			...
			},
			{
				.m_Name = "Compaction",
				.m_Value =
					"64-lane group-shared stable scan",
			},
			{
				.m_Name = "GPU schedules",
				.m_Value = std::to_string(
					m_DebugReadback->
						GetScheduledCount()),
			},
			{
		...
		diagnostics.m_Checks = {
			{
				.m_Name =
					"GPU tile readback",
				.m_Status = !result.m_IsValid ?
					LabDiagnosticCheckStatus::
						Pending :
					LabDiagnosticCheckStatus::
						Passed,
				.m_Detail =
					"Header and indices are copied from the completed GPU cull frame.",
			},
			{
				.m_Name =
					"Fixed-stride address/count",
				.m_Status = !result.m_IsValid ?
					LabDiagnosticCheckStatus::
						Pending :
					headerValid ?
						...
						LabDiagnosticCheckStatus::
							Failed,
				.m_Detail =
					"Offset equals TileIndex * 64 and count does not exceed 64.",
			},
			{
				.m_Name =
					"Selected fixture result",
				.m_Status = !result.m_IsValid ?
					LabDiagnosticCheckStatus::
						Pending :
					fixtureResultValid ?
	...
	{
		return LabId(
			"gglab.lab.forward_plus");
	}

	LabDescriptor
		ForwardPlusLabSession::GetDescriptor() noexcept
	{
		return {
			.m_Id = GetId(),
			.m_DisplayName = "Forward+",
			.m_Category = "Rendering",
			.m_Description = "Validates fixed-stride 16x16 tile culling, background rejection, near-plane lights, stable light order, and actual GPU header/index readback.",
			.m_Kind = LabKind::Pipeline,
			.m_SchemaVersion = 1,
		};
	}

	std::unique_ptr<LabSessionBase>
		ForwardPlusLabSession::Create(
			const LabSessionCreateInfo& createInfo) noexcept
	{
		return std::make_unique<
			ForwardPlusLabSession>(createInfo);
	}
}
