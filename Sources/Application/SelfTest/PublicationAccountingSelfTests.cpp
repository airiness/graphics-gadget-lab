...
namespace gglab
{
	namespace
	{
		void RunStepAccountingTests(SelfTestContext& context) noexcept
		{
			...
			context.Check(
				payload.RetireStep(copied) == 0 &&
					payload.m_RemainingSourceBytes == 16,
			...
			context.Check(
				payload.RetireStep(released) == 6 &&
					payload.m_RemainingSourceBytes == 10,
			...
			context.Check(
				payload.RetireStep(mixed) == 4 &&
					payload.m_RemainingSourceBytes == 6,
		...
		void RunTerminalAccountingTests(SelfTestContext& context) noexcept
		{
			...
			context.Check(
				completed.RetireTerminal() == 12 &&
					completed.m_RemainingSourceBytes == 0,
				"Terminal publication retires all remaining source bytes");
			context.Check(
				completed.RetireTerminal() == 0,
			...
			context.Check(
				stepRetired == 0 && terminalRetired == 9 &&
					failed.m_RemainingSourceBytes == 0,
