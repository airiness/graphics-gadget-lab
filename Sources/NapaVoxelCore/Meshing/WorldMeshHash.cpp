...
namespace napa::voxel
{
	namespace
	{
		[[nodiscard]] bool AccumulateCount(
			std::uint64_t value,
			std::uint64_t& total) noexcept
		{
			const std::optional<std::uint64_t> sum =
				CheckedAdd(total, value);
			if (!sum)
			{
				return false;
			}
	...
	ValidationResult ValidateAndHashWorldMeshRecords(
		...
		WorldMeshValidationResult& result)
	{
		...
		if (!expectedChunkCount ||
			records.size() != *expectedChunkCount)
		{
			return {
		...
		for (const ChunkMeshRecord& record : records)
		{
			...
			if (!AccumulateCount(
					chunkValidation.m_VertexCount,
					validated.m_VertexCount) ||
				!AccumulateCount(
					chunkValidation.m_SectionCount,
					validated.m_SectionCount) ||
				!AccumulateCount(
					chunkValidation.m_IndexCount,
					validated.m_IndexCount) ||
				!AccumulateCount(
					chunkValidation.m_TriangleCount,
					validated.m_TriangleCount) ||
				!AccumulateCount(
					record.m_SkippedDegenerateTriangleCount,
					validated
