...
{
	inline constexpr double MeshPositionQuantizationScale = CanonicalPositionQuantizationScale;
	...
	struct QuantizedMeshPosition
	{
		...
		[[nodiscard]] friend constexpr bool operator==(
			const QuantizedMeshPosition&, const QuantizedMeshPosition&) noexcept = default;
	};
	...
	struct QuantizedMeshNormal
	{
		...
		[[nodiscard]] friend constexpr bool operator==(
			const QuantizedMeshNormal&, const QuantizedMeshNormal&) noexcept = default;
	};
	...
	struct QuantizedMeshAabb
	{
		...
		[[nodiscard]] friend constexpr bool operator==(
			const QuantizedMeshAabb&, const QuantizedMeshAabb&) noexcept = default;
	};
	...
	struct MeshTriangleWindingEvidence
	{
		...
		[[nodiscard]] friend constexpr bool operator==(const MeshTriangleWindingEvidence&,
	...
	struct MeshValidationResult
	{
		...
		[[nodiscard]] friend constexpr bool operator==(
			const MeshValidationResult&, const MeshValidationResult&) noexcept = default;
	};
	...
	public:
		...
		[[nodiscard]] bool ContainsTargetCellDomain(QuantizedMeshPosition position) const noexcept;

	private:
		friend ValidationResult PrepareMeshQuantizationContext(const VoxelWorldConfig& config,
			ChunkCoord chunk, MeshQuantizationContext& context) noexcept;
		friend ValidationResult QuantizeMeshPosition(Float3 position,
			const MeshQuantizationContext& context, QuantizedMeshPosition& quantized) noexcept;
	...
	[[nodiscard]] ValidationResult PrepareMeshQuantizationContext(const VoxelWorldConfig& config,
		ChunkCoord chunk, MeshQuantizationContext& context) noexcept;
	// Input positions use target Chunk-local physical space.
	[[nodiscard]] ValidationResult QuantizeMeshPosition(Float3 position,
		const MeshQuantizationContext& context, QuantizedMeshPosition& quantized) noexcept;
	[[nodiscard]] ValidationResult QuantizeMeshNormal(
		Float3 normal, QuantizedMeshNormal& quantized) noexcept;
