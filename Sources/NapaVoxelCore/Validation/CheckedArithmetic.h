...
namespace napa::voxel
{
	...
	[[nodiscard]] constexpr std::optional<Integer> CheckedAdd(
		Integer lhs,
	...
	template<std::integral Integer>
		requires (!std::same_as<std::remove_cv_t<Integer>, bool>)
	[[nodiscard]] constexpr std::optional<Integer> CheckedMul(
		Integer lhs,
		Integer rhs) noexcept
	{
		using Limits = std::numeric_limits<Integer>;
	...
	template<std::integral Target, std::integral Source>
		requires (
			!std::same_as<std::remove_cv_t<Target>, bool> &&
			!std::same_as<std::remove_cv_t<Source>, bool>)
	[[nodiscard]] constexpr std::optional<Target> CheckedNarrow(
