...
namespace gglab
{
	std::unique_ptr<DX12PipelineState> StreamPSOCreator::CreateGraphicsPSO(
		DX12Device* dx12Device, const GraphicsPipelineDesc& desc)
	{
		if (!desc.Validate())
		{
	...
	std::unique_ptr<DX12PipelineState> StreamPSOCreator::CreateComputePSO(
		DX12Device* dx12Device, const ComputePipelineDesc& desc)
	{
		if (!desc.Validate())
		{
	...
	D3D12_PIPELINE_STATE_STREAM_DESC StreamPSOCreator::BuildGraphicsStream(
		const GraphicsPipelineDesc& desc, StreamWriter& writer) noexcept
	{
		...
		writer.Add(rs);
		writer.AddIf(desc.m_InputLayoutDesc.NumElements > 0,
			CD3DX12_PIPELINE_STATE_STREAM_INPUT_LAYOUT(desc.m_InputLayoutDesc));
		writer.Add(CD3DX12_PIPELINE_STATE_STREAM_PRIMITIVE_TOPOLOGY(desc.m_Topology));
		writer.AddIf(desc.m_VertexShader.BytecodeLength > 0,
			CD3DX12_PIPELINE_STATE_STREAM_VS(desc.m_VertexShader));
		writer.AddIf(desc.m_PixelShader.BytecodeLength > 0,
			CD3DX12_PIPELINE_STATE_STREAM_PS(desc.m_PixelShader));
		writer.AddIf(desc.m_HullShader.BytecodeLength > 0,
			CD3DX12_PIPELINE_STATE_STREAM_HS(desc.m_HullShader));
		writer.AddIf(desc.m_DomainShader.BytecodeLength > 0,
			CD3DX12_PIPELINE_STATE_STREAM_DS(desc.m_DomainShader));
		writer.AddIf(desc.m_GeometryShader.BytecodeLength > 0,
			CD3DX12_PIPELINE_STATE_STREAM_GS(desc.m_GeometryShader));
		writer.AddIf(desc.m_Formats.m_DsvFormat != DXGI_FORMAT_UNKNOWN,
			CD3DX12_PIPELINE_STATE_STREAM_DEPTH_STENCIL_FORMAT(desc.m_Formats.m_DsvFormat));
		writer.Add(
			CD3DX12_PIPELINE_STATE_STREAM_RENDER_TARGET_FORMATS(desc.m_Formats.m_RtvFormats));
		...
		writer.AddIf((desc.m_Formats.m_SampleCount > 1 || desc.m_Formats.m_SampleQuality > 0),
			CD3DX12_PIPELINE_STATE_STREAM_SAMPLE_DESC(
				{ desc.m_Formats.m_SampleCount, desc.m_Formats.m_SampleQuality }));
	...
	D3D12_PIPELINE_STATE_STREAM_DESC StreamPSOCreator::BuildComputeStream(
