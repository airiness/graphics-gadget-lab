...
namespace gglab
{
		...
		public:
			void U32(uint32_t value)
			{
				for (uint32_t i = 0; i < 4; ++i)
					m_Data.push_back(static_cast<std::byte>((value >> (i * 8)) & 0xffu));
			}
		...
		private:
			bool Unsigned(uint64_t& value, size_t width) noexcept
			{
				...
				for (size_t i = 0; i < width; ++i)
					value |= static_cast<uint64_t>(std::to_integer<uint8_t>(m_Data[m_Offset + i]))
					<< (i * 8);
				m_Offset += width;
				return true;
			}
	...
	std::vector<std::byte> TextureArtifactCodec::Serialize(const TextureArtifact& artifact) noexcept
	{
		if (!artifact.IsValid())
			return {};
		...
		TextureAssetValidationLimits limits) noexcept
	{
		...
		if (!reader.U32(schema) || schema != TextureArtifactSchemaVersion ||
			!reader.U32(resourceFormat) || !reader.U32(viewFormat) || !reader.U32(srvDimension) ||
			!reader.U32(texture.m_Extent.m_Width) || !reader.U32(texture.m_Extent.m_Height) ||
			!reader.U32(texture.m_Extent.m_Depth) || !reader.U32(arraySize) ||
			!reader.U32(mipLevels) || !reader.U32(colorSpace) || !reader.U64(subresourceCount) ||
			!reader.U64(pixelBytes) || arraySize > std::numeric_limits<uint16_t>::max() ||
		...
		if (!metadataValidation.IsValid())
		{
			...
			result.m_Error = std::format("Texture DDC metadata failed validation: {}.",
		...
		TextureArtifactBuildResult built = CreateTextureArtifact(std::move(texture), limits);
		if (!built.Succeeded())
		{
			result = {};
			result.m_StructureError = built.m_StructureError;
			result.m_Error =
				built.m_Error == TextureArtifactBuildError::InvalidStructure
				? std::format("Texture DDC artifact failed structural validation: {}.",
