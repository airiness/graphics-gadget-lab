...
namespace gglab
{
	namespace
	{
			...
			TextureSemantic semantic) noexcept
		{
			...
			case TextureSemantic::GenericColor:
				return "GenericColor";
			case TextureSemantic::GenericData:
				return "GenericData";
			}
			return "Unknown";
		}

		[[nodiscard]] std::string TextureDebugSourcePath(const std::filesystem::path& sourcePath)
		{
			if (sourcePath.empty())
			{
				return {};
			...
			return std::format("External/{}#{:08X}", sourcePath.filename().generic_string(),
			...
			const TextureAssetData& textureData) noexcept
		{
			...
			return {
				...
				.m_OperationCount =
					std::max<uint32_t>(1, static_cast<uint32_t>(textureData.m_Subresources.size())),
			};
		}

		[[nodiscard]] constexpr AssetStateEventOperationPhase ResidencyStateEventPhase(
			const AssetResidencyOperation& operation, bool completes = false) noexcept
		{
		...
		AssetStateEventOperationPhase operationPhase) noexcept
	{
		...
		if ((previousState != state || operationPhase == AssetStateEventOperationPhase::Completes))
		{
			m_StateEvents->Push(
				{
					.m_ContentVersion =
						MakeAssetContentVersion(texture.m_Id, texture.m_ContentGeneration),
	...
	void TextureAssetSystem::InitializeReservedTextures() noexcept
	{
		...
		for (BuiltinTextureAsset& builtinTexture : builtinTextures)
		{
			...
			uploads.emplace_back(
				MakeTextureUploadData(id, std::move(builtinTexture.m_Data), importSettings));
		}
		...
		if (!uploads.empty())
		{
			...
			GGLAB_ASSERT_MSG(
				uploadsSucceeded, "TextureAssetSystem failed to initialize reserved textures.");
		}

		const auto queueOptionalTexture = [this](ReservedTextureIDIndex idIndex,
			...
			const std::filesystem::path& path) noexcept
			{
				...
				const bool inserted = m_Store.BindCacheKey(canonicalPath, importSettings, textureId);
				GGLAB_ASSERT_MSG(
					inserted, "TextureAssetSystem failed to register an optional reserved texture.");
				GGLAB_UNUSED(QueueTextureLoad(
					textureId, canonicalPath, importSettings, TaskPriority::Background));
			};

		queueOptionalTexture(ReservedTextureIDIndex::UVTestTexture1K, "UVTestTexture1K",
			"Assets/Textures/UVTest1K.png");
		queueOptionalTexture(ReservedTextureIDIndex::UVTestTexture4K, "UVTestTexture4K",
			"Assets/Textures/UVTest4K.png");
	}

	void TextureAssetSystem::Finalize(const RHIFencePoint& fencePoint) noexcept
	{
		GGLAB_ASSERT_MSG(m_PublicationOrphanedTextures.empty(),
		...
		const std::filesystem::path& path, TextureSemantic semantic, TaskPriority priority) noexcept
	{
		...
		if (const TextureID existing = FindTexture(canonicalPath, importSettings);
			existing.IsValid())
		{
				...
				texture->m_State != AssetState::Cancelled)
			{
				const TaskHandle task =
					texture->m_State == AssetState::CpuReady && !texture->m_Load.m_IsReloading
					? TaskHandle{}
			...
			GGLAB_LOG_GRAPHICS_INFO(
				...
				existing.Value(), canonicalPath.string());
		}
		...
		const TaskHandle task =
			QueueTextureLoad(textureId, canonicalPath, importSettings, priority);
		return {
	...
	TaskHandle TextureAssetSystem::QueueTextureLoad(TextureID textureId,
		const std::filesystem::path& canonicalPath, const TextureImportSettings& importSettings,
		TaskPriority priority, bool residencyReload,
		AssetResidencyOperation residencyOperation) noexcept
	{
		...
		GGLAB_ASSERT_MSG(texture->m_Source.m_CanonicalPath == canonicalPath &&
			texture->m_Source.m_ImportSettings == importSettings,
			"Texture decode request does not match the texture source identity.");
		ProgressReporter(texture->m_Load.m_Progress)
			.Report(
				0.05f, "Queued for texture decoding", canonicalPath.filename().generic_string());
		const AssetLoadSubmission submission = m_LoadCoordinator->SubmitTextureDecode({
			.m_ContentVersion = MakeAssetContentVersion(textureId, texture->m_ContentGeneration),
			...
			.m_ExpectedSourceDigest =
				residencyReload ? texture->m_Source.m_SourceDigest : SourceDigest{},
			.m_ExpectedDerivedDataKey =
				residencyReload ? texture->m_Source.m_DerivedDataKey : DerivedDataKey{},
		...
		{
			SetTextureState(*texture, residencyReload ? AssetState::CpuReady : AssetState::Failed,
				residencyOperation, ResidencyStateEventPhase(residencyOperation, true));
			texture->m_Load.m_IsReloading = false;
			ProgressReporter(texture->m_Load.m_Progress)
				.Report(0.05f, "Texture decode submission failed",
		...
		const AssetResidencyOperation& operation) const noexcept
	{
		...
		const Texture* texture =
			GetTexture(TextureID{ static_cast<uint32_t>(contentVersion.m_Key.m_StableId) });
		...
		const AssetResidencyOperation& operation) noexcept
	{
		const AssetContentVersion contentVersion = operation.m_Token.m_ContentVersion;
		Texture* texture =
			EditTexture(TextureID{ static_cast<uint32_t>(contentVersion.m_Key.m_StableId) });
	...
	TextureAssetSystem::EvictionFinalizationResult TextureAssetSystem::FinalizeEviction(
		const AssetResidencyOperation& operation, bool protectedByInterest) noexcept
	{
		const AssetContentVersion contentVersion = operation.m_Token.m_ContentVersion;
		Texture* texture =
			EditTexture(TextureID{ static_cast<uint32_t>(contentVersion.m_Key.m_StableId) });
		...
		{
			SetTextureState(
				*texture, AssetState::Ready, operation, AssetStateEventOperationPhase::Completes);
	...
	AssetResidencyOperation TextureAssetSystem::BeginResidencyOperation(
		const AssetContentVersion& contentVersion, AssetResidencyOperationKind kind,
		AssetResidencyController& controller) noexcept
	{
		...
		const AssetResidencyOperation operation =
			controller.BeginResidencyOperation(*texture, contentVersion, kind);
		if (operation.IsValid() && kind == AssetResidencyOperationKind::Evict)
		{
			SetTextureState(*texture, AssetState::Evicting, operation,
				AssetStateEventOperationPhase::InProgress);
		}
		return operation;
	}

	TaskHandle TextureAssetSystem::RequestResidency(TextureID textureId, uint64_t generation,
		TaskPriority priority, AssetResidencyController& controller) noexcept
	{
		Texture* texture = EditTexture(textureId);
		...
		if (texture->m_State == AssetState::Evicting)
		{
			const AssetResidencyOperation operation = controller.BeginResidencyOperation(
				*texture, contentVersion, AssetResidencyOperationKind::Reload);
			SetTextureState(
				*texture, AssetState::Ready, operation, AssetStateEventOperationPhase::Completes);
			return {};
		}
		...
		if (texture->m_Load.m_IsReloading)
		{
			operation = {
				.m_Token =
					MakeAssetOperationToken(contentVersion, texture->m_ResidencyOperationSerial),
		...
		TextureID textureId, AssetResidencyPolicy policy) noexcept
	{
		...
		return texture && AssetResidencyController::SetResidencyPolicy(
			*texture, policy, IsReservedTextureId(textureId));
	}
	...
	uint32_t TextureAssetSystem::ResolveSrvIndex(
	...
	{
		const Texture* texture = GetTexture(content.m_Id);
		if (!content.IsValid() || !texture || texture->m_ContentGeneration != content.m_Generation)
		{
			return std::nullopt;
		}
	...
	std::vector<TextureAssetReadInfo> TextureAssetSystem::GetTextureAssetReadInfos() const
	{
		...
		for (const auto& [textureId, texture] : m_Store.Entries())
		{
			infos.push_back({
				...
				.m_Texture = texture->m_Gpu.m_Texture,
				.m_DebugName =
					m_Device ? std::string(m_Device->GetTextureDebugName(texture->m_Gpu.m_Texture))
							 : std::string{},
				...
				.m_IsCpuArtifactCached =
					m_ArtifactCache->Contains(texture->m_Source.m_ArtifactContentDigest),
	...
	{
		return static_cast<uint32_t>(std::ranges::count_if(m_Store.Entries() | std::views::values,
		...
		const TextureImportSettings& importSettings) noexcept
	{
		...
		const bool cacheKeyBound = m_Store.BindCacheKey(canonicalPath, importSettings, textureId);
		GGLAB_ASSERT_MSG(
			cacheKeyBound, "TextureAssetSystem: emplace path and TextureID pair failed.");
		if (!cacheKeyBound)
		{
			return InvalidTextureID;
		}
		...
		GGLAB_ASSERT_MSG(
			inserted, "TextureAssetSystem: emplace TextureID and Texture pair failed.");
		if (!inserted)
		{
		...
		BeginAssetContentGeneration(*insertedTexture, 1, AssetState::CpuReady);
	...
	TextureAssetSystem::TextureUploadData TextureAssetSystem::MakeTextureUploadData(
		TextureID textureId, TextureAssetData&& textureData,
		const TextureImportSettings& importSettings,
		AssetContentFingerprint contentFingerprint) noexcept
	{
		if (!contentFingerprint.IsValid())
		{
			contentFingerprint = ComputeTextureContentFingerprint(textureData, importSettings);
		}
		TextureArtifactHandle artifactHandle =
			m_ArtifactCache->CreateAndAdmit(std::move(textureData));
		if (!artifactHandle)
		{
			return {};
		}
		return MakeTextureUploadData(
			textureId, std::move(artifactHandle), importSettings, contentFingerprint);
	}

	TextureAssetSystem::TextureUploadData TextureAssetSystem::MakeTextureUploadData(
		TextureID textureId, TextureArtifactHandle artifact,
		const TextureImportSettings& importSettings, AssetContentFingerprint contentFingerprint,
		SourceDigest sourceDigest, DerivedDataKey derivedDataKey) noexcept
	{
		...
		if (!contentFingerprint.IsValid())
		{
			contentFingerprint = ComputeTextureContentFingerprint(artifact->m_Data, importSettings);
		}
		if (!contentFingerprint.IsValid())
		{
			return {};
		...
		uploadData.m_ColorSpace = GetTextureColorSpaceFromSemantic(importSettings.m_Semantic);
	...
	bool TextureAssetSystem::UploadTexture(const TextureUploadData& uploadData,
		TransferBatch& transferBatch, AssetResidencyOperation residencyOperation) noexcept
	{
		auto* texture = EditTexture(uploadData.m_TextureId);
		GGLAB_ASSERT_MSG(
			texture != nullptr, "TextureAssetSystem::UploadTexture: invalid TextureID.");
		if (!texture)
		{
			return false;
		}
		GGLAB_ASSERT_MSG(texture->m_Source.m_ImportSettings == uploadData.m_ImportSettings,
		...
		SetTextureState(*texture, AssetState::UploadQueued, residencyOperation, operationPhase);
		...
		if (residencyOperation.IsValid() && texture->m_Source.m_ArtifactContentDigest.IsValid() &&
			uploadData.m_Artifact->m_ContentDigest != texture->m_Source.m_ArtifactContentDigest)
		{
			GGLAB_LOG_GRAPHICS_ERROR(
		...
		ProgressReporter(texture->m_Load.m_Progress)
			.Report(0.68f, "Recording texture upload",
				std::format("{}x{}, {} mip levels, {} subresources", textureData.m_Extent.m_Width,
					textureData.m_Extent.m_Height, textureData.m_MipLevels,
		...
		const std::string category = std::format(
			"Texture.{}", TextureSemanticDebugText(uploadData.m_ImportSettings.m_Semantic));
		const std::string source = TextureDebugSourcePath(texture->m_Source.m_CanonicalPath);
		...
		texture->m_Source.m_ArtifactContentDigest = uploadData.m_Artifact->m_ContentDigest;
		...
		GGLAB_ASSERT_MSG(texture->m_Gpu.m_Srv.IsValid(),
			"TextureAssetSystem::UploadTexture: failed to create RHI texture SRV.");
		...
		SetTextureState(*texture, AssetState::GpuProcessing, residencyOperation, operationPhase);
		return true;
	}

	void TextureAssetSystem::CompleteTextureUpload(
		TextureID textureId, bool succeeded, AssetResidencyOperation residencyOperation) noexcept
	{
		...
		SetTextureState(*texture,
			...
			residencyOperation, ResidencyStateEventPhase(residencyOperation, residencyReload));
		ProgressReporter(texture->m_Load.m_Progress)
	...
	bool TextureAssetSystem::QueueTextureUpload(TextureUploadData&& uploadData,
		TaskPriority priority, AssetResidencyOperation residencyOperation) noexcept
	{
		...
		{
			SetTextureState(*texture, AssetState::CpuReady, residencyOperation,
				ResidencyStateEventPhase(residencyOperation));
		}
		ProgressReporter(texture->m_Load.m_Progress)
			.Report(0.62f, "Waiting for texture upload admission",
		...
		m_AssetUploadScheduler->EnqueueUploadRecording(
			...
			[this, textureId, generation, priority, residencyOperation, payload]() mutable noexcept
			{
				...
				if (residencyOperation.IsValid() && !AssetResidencyController::IsCurrentOperation(
					*currentTexture, residencyOperation))
				{
					return;
				}
				if (currentTexture->m_Load.m_CancelRequested)
				{
					...
					SetTextureState(*currentTexture,
				...
				if (!RecordTextureUpload(
					textureId, generation, priority, std::move(*payload), residencyOperation))
				{
					const bool residencyReload = currentTexture->m_Load.m_IsReloading;
					SetTextureState(*currentTexture,
						residencyReload ? AssetState::CpuReady : AssetState::Failed,
						...
						ResidencyStateEventPhase(residencyOperation, residencyReload));
					currentTexture->m_Load.m_IsReloading = false;
					ProgressReporter(currentTexture->m_Load.m_Progress)
						.Report(0.68f, "Texture upload recording failed");
				}
			});
		return true;
	}

	bool TextureAssetSystem::RecordTextureUpload(TextureID textureId, uint64_t generation,
		TaskPriority priority, TextureUploadData uploadData,
		AssetResidencyOperation residencyOperation) noexcept
	{
		Texture* texture = EditTexture(textureId);
		if (!texture || texture->m_ContentGeneration != generation)
		...
		const AssetUploadHandle uploadHandle = m_AssetUploadScheduler->RecordUpload(
				...
				const AssetUploadCompletionInfo& completion) noexcept
			{
				const Texture* texture = GetTexture(textureId);
				if (!texture || texture->m_ContentGeneration != generation ||
					(residencyOperation.IsValid() && !AssetResidencyController::IsCurrentOperation(
						*texture, residencyOperation)))
				{
					return;
				}
				...
				if (cancelled)
				{
					GGLAB_LOG_GRAPHICS_INFO(
						...
						textureId.Value(), completion.m_ElapsedMilliseconds);
				}
				else if (succeeded)
				{
					GGLAB_LOG_GRAPHICS_INFO("Texture {} GPU upload completed in {:.2f} ms.",
						textureId.Value(), completion.m_ElapsedMilliseconds);
				}
				else
				{
	...
	void TextureAssetSystem::RouteTextureDecodeCompletion(TextureDecodeSucceeded&& result) noexcept
	{
		...
		const TextureID textureId{
			static_cast<uint32_t>(result.m_Operation.m_ContentVersion.m_Key.m_StableId) };
		const uint64_t generation = result.m_Operation.m_ContentVersion.m_ContentGeneration;
		const Texture* texture = GetTexture(textureId);
		if (!texture || texture->m_ContentGeneration != generation ||
			texture->m_Load.m_CancelRequested ||
			(result.m_ResidencyReload && !AssetResidencyController::IsCurrentOperation(
				*texture, result.m_ResidencyOperation)))
		{
		...
		GGLAB_LOG_GRAPHICS_INFO("Texture {} derived data resolved (cache={}, key={}).",
			textureId.Value(), result.m_DerivedDataCacheHit ? "hit" : "miss",
		...
		m_AssetUploadScheduler->EnqueueCpuPayload(
			...
			[this, operation, semantic, completion, payload, contentFingerprint, sourceDigest,
			derivedDataKey, residencyReload, residencyOperation]() mutable noexcept
			{
				CompleteTextureLoad(operation, semantic, completion, std::move(payload),
					contentFingerprint, sourceDigest, derivedDataKey, residencyReload,
					residencyOperation);
			});
	}

	void TextureAssetSystem::RouteTextureDecodeCompletion(TextureDecodeFailed&& result) noexcept
	{
		CompleteTextureLoad(result.m_Operation, result.m_Semantic, result.m_Completion, {}, {}, {},
			{}, result.m_ResidencyReload, result.m_ResidencyOperation);
	}

	void TextureAssetSystem::CompleteTextureLoad(AssetOperationToken operation,
		TextureSemantic semantic, const TaskCompletionInfo& completion,
		TextureArtifactHandle artifact, AssetContentFingerprint contentFingerprint,
		SourceDigest sourceDigest, DerivedDataKey derivedDataKey, bool residencyReload,
		AssetResidencyOperation residencyOperation) noexcept
	{
		...
		GGLAB_ASSERT_MSG(texture->m_Source.m_ImportSettings.m_Semantic == semantic,
		...
		{
			SetTextureState(*texture,
				residencyReload ? AssetState::CpuReady : AssetState::Cancelled, residencyOperation,
				ResidencyStateEventPhase(residencyOperation, residencyReload));
			texture->m_Load.m_IsReloading = false;
			ProgressReporter(texture->m_Load.m_Progress)
				.Report(0.05f, "Texture loading cancelled", completion.m_Name);
			return;
		}
		...
		if (completion.m_Status != TaskStatus::Succeeded)
		{
			...
			texture->m_Load.m_IsReloading = false;
			ProgressReporter(texture->m_Load.m_Progress)
				.Report(0.05f, "Texture decoding failed", completion.m_Error);
			GGLAB_LOG_GRAPHICS_ERROR(
				"Async texture decode '{}' failed: {}", completion.m_Name, completion.m_Error);
			return;
		}
		if (residencyReload && texture->m_Source.m_ArtifactContentDigest.IsValid() &&
			(!artifact || artifact->m_ContentDigest != texture->m_Source.m_ArtifactContentDigest))
		{
			const ArtifactContentDigest actualDigest =
				artifact ? artifact->m_ContentDigest : ArtifactContentDigest{};
			SetTextureState(*texture, AssetState::CpuReady, residencyOperation,
			...
			GGLAB_LOG_GRAPHICS_ERROR(
				...
				generation, ArtifactContentDigestText(texture->m_Source.m_ArtifactContentDigest),
		...
		SetTextureState(*texture, AssetState::CpuReady, residencyOperation,
			ResidencyStateEventPhase(residencyOperation));
		ProgressReporter(texture->m_Load.m_Progress)
			.Report(0.62f, "Queued for texture upload publication", completion.m_Name);
		auto uploadData = MakeTextureUploadData(textureId, std::move(artifact),
			texture->m_Source.m_ImportSettings, contentFingerprint, sourceDigest, derivedDataKey);
		if (!QueueTextureUpload(std::move(uploadData), completion.m_Priority, residencyOperation))
		{
			SetTextureState(*texture, residencyReload ? AssetState::CpuReady : AssetState::Failed,
				residencyOperation, ResidencyStateEventPhase(residencyOperation, residencyReload));
			texture->m_Load.m_IsReloading = false;
			ProgressReporter(texture->m_Load.m_Progress)
				.Report(0.62f, "Texture upload queueing failed");
			return;
		}
		GGLAB_LOG_GRAPHICS_INFO(
			"Async texture {} queued for upload admission (queueMs={:.2f}, cpuMs={:.2f}).",
			textureId.Value(), completion.m_QueueMilliseconds, completion.m_ExecutionMilliseconds);
	}

	void TextureAssetSystem::CancelTextureIfUnreferenced(
		TextureID textureId, uint64_t generation) noexcept
	{
		Texture* texture = EditTexture(textureId);
		if (!texture || texture->m_ContentGeneration != generation ||
			texture->m_State == AssetState::Ready || texture->m_State == AssetState::Failed ||
			texture->m_State == AssetState::Cancelled)
		{
			return;
		}
		...
		if (texture->m_Load.m_IsReloading && !texture->m_Gpu.m_Texture.IsValid())
		{
			...
			ProgressReporter(texture->m_Load.m_Progress)
				.Report(1.0f, "Texture residency reload cancelled", texture->m_DebugLabel);
			return;
		}
		SetTextureState(*texture,
			texture->m_Gpu.m_Texture.IsValid() ? AssetState::GpuProcessing : AssetState::Cancelled);
		ProgressReporter(texture->m_Load.m_Progress)
			.Report(0.96f,
				texture->m_Gpu.m_Texture.IsValid() ? "Texture cancellation pending GPU completion"
		...
		TextureID textureId, uint64_t generation) noexcept
	{
		...
		if (texture->m_Gpu.m_Texture.IsValid() && texture->m_State != AssetState::Ready)
		{
			...
			ProgressReporter(texture->m_Load.m_Progress)
				.Report(0.96f, "Texture publication rollback pending GPU completion",
					texture->m_DebugLabel);
			return;
		}
		...
		TextureID textureId, uint64_t generation, TaskPriority priority) noexcept
	{
		...
		GGLAB_UNUSED(m_LoadCoordinator->UpdateTextureDecodePriority(
			MakeAssetContentVersion(textureId, generation), priority));
		...
		TextureID textureId, uint64_t generation) noexcept
	{
		...
		texture->m_Load.m_CancelRequested = false;
		ProgressReporter(texture->m_Load.m_Progress)
			.Report(0.82f, "Texture cancellation rescinded",
				"A new owner acquired the in-flight texture");
	}

	TaskHandle TextureAssetSystem::RequestTextureResidency(
		TextureID textureId, AssetResidencyOperation operation, TaskPriority priority) noexcept
	{
		Texture* texture = EditTexture(textureId);
		...
		if (TextureArtifactHandle artifact =
			m_ArtifactCache->Find(texture->m_Source.m_ArtifactContentDigest))
		{
			ProgressReporter(texture->m_Load.m_Progress)
				.Report(0.62f, "Texture CPU artifact cache hit",
					ArtifactContentDigestText(artifact->m_ContentDigest));
			TextureUploadData uploadData = MakeTextureUploadData(textureId, std::move(artifact),
				texture->m_Source.m_ImportSettings, texture->m_Source.m_ContentFingerprint,
				texture->m_Source.m_SourceDigest, texture->m_Source.m_DerivedDataKey);
			if (QueueTextureUpload(std::move(uploadData), priority, operation))
			{
				return {};
			}
		}
		...
		{
			texture->m_Load.m_IsReloading = false;
			SetTextureState(*texture, AssetState::CpuReady, operation,
				AssetStateEventOperationPhase::Completes);
			ProgressReporter(texture->m_Load.m_Progress)
				.Report(0.05f, "Texture residency reload unavailable",
					"CPU artifact cache miss and no source path");
			return {};
		}
		const TaskHandle task = QueueTextureLoad(textureId, texture->m_Source.m_CanonicalPath,
			texture->m_Source.m_ImportSettings, priority, true, operation);
		if (!task.IsValid())
		{
			texture->m_Load.m_IsReloading = false;
		}
	...
	bool TextureAssetSystem::FinalizeTextureEviction(
		TextureID textureId, AssetResidencyOperation operation) noexcept
	{
		Texture* texture = EditTexture(textureId);
		...
		texture->m_Gpu.m_SrvDimension = RHITextureViewDimension::Unknown;
		SetTextureState(
			*texture, AssetState::CpuReady, operation, AssetStateEventOperationPhase::Completes);
		ProgressReporter(texture->m_Load.m_Progress)
			.Report(1.0f, "Texture GPU residency released", texture->m_DebugLabel);
		return true;
	}

	void TextureAssetSystem::CreateTextureEntry(TextureID id, std::string_view textureName,
		TextureImportSettings importSettings, const std::filesystem::path& sourcePath) noexcept
	{
		if (GetTexture(id) == nullptr)
		{
			...
			if (m_Store.Insert(id, std::move(texture)))
			{
				...
				BeginAssetContentGeneration(*insertedTexture, 1, AssetState::CpuReady,
					IsReservedTextureId(id) ? AssetResidencyPolicy::Pinned
