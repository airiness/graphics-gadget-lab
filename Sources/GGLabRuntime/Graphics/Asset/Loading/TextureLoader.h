...
namespace gglab
{
	...
	{
	public:
		[[nodiscard]] static TextureAssetData LoadTextureData(const std::filesystem::path& texPath,
		...
		[[nodiscard]] static TextureAssetData LoadTextureData(
			const std::filesystem::path& sourceIdentity, std::span<const std::byte> sourceBytes,
			const TextureImportSettings& settings, const ProgressReporter& progress = {}) noexcept;

		[[nodiscard]] static TextureAssetData LoadTextureData(const std::filesystem::path& texPath,
			TextureColorSpace colorSpace, const ProgressReporter& progress = {}) noexcept;

		[[nodiscard]] static TextureAssetData MakeTexture2DRgba8(uint32_t width, uint32_t height,
