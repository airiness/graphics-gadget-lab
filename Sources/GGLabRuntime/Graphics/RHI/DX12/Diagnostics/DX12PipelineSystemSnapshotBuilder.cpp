...
namespace gglab
{
		...
		const PipelineCache* pipelineCache, RHIPipelineSystemSnapshot& outSnapshot) noexcept
	{
		...
		for (const auto& binding : system.m_BindingLayouts)
		{
			...
			layout.m_BackendLayoutId = binding.m_Handle.Index();
			layout.m_Slots.reserve(binding.m_SlotCount);

			int32_t backendParameter = 0;
			for (uint32_t slotIndex = 0; slotIndex < binding.m_SlotCount; ++slotIndex)
			{
				...
				else if (source.m_Type != RHIBindingType::Unknown)
				{
					...
					slot.m_BackendParameterIndex = backendParameter++;
					slot.m_BackendBindingType = BackendBindingType(source);
				}
				layout.m_Slots.push_back(std::move(slot));
			}
			layout.m_BackendParameterCount = static_cast<uint32_t>(backendParameter);
		...
		for (uint32_t index = 0; index < system.m_Pipelines.size(); ++index)
		{
			...
			pipeline.m_BackendPipelineId = pipeline.m_Handle.Index();
			pipeline.m_BackendLayoutId =
