...
namespace gglab
{
	...
	public:
		...
		void SetRenderTargets(std::span<DX12DescriptorView> rtDescriptors,
			const DX12DescriptorView& dsDescriptor) const noexcept;
		void SetRenderTarget(const DX12DescriptorView& rtDsescriptor,
			const DX12DescriptorView& dsDescriptor = {}) const noexcept;
		void SetVertexBuffers(uint32_t startSlot,
			std::span<D3D12_VERTEX_BUFFER_VIEW> vertexBufferViews) const noexcept;
		...
		void SetGraphicsDescriptor(
			uint32_t parameterIndex, const DX12DescriptorView& descriptor) const noexcept;
		void SetGraphicsRoot32BitConstant(
			uint32_t parameterIndex, uint32_t value, uint32_t destOffset = 0) const noexcept;
		void SetGraphicsRoot32BitConstants(uint32_t parameterIndex,
			std::span<const uint32_t> values, uint32_t destOffset = 0) const noexcept;
		void SetComputeConstantBuffer(
			uint32_t parameterIndex, D3D12_GPU_VIRTUAL_ADDRESS gpuAddress) const noexcept;
		void SetComputeReadOnlyBuffer(
			uint32_t parameterIndex, D3D12_GPU_VIRTUAL_ADDRESS gpuAddress) const noexcept;
		void SetComputeReadWriteBuffer(
			uint32_t parameterIndex, D3D12_GPU_VIRTUAL_ADDRESS gpuAddress) const noexcept;
		void SetComputeDescriptorTable(
			uint32_t parameterIndex, D3D12_GPU_DESCRIPTOR_HANDLE table) const noexcept;
		void SetComputeRoot32BitConstants(uint32_t parameterIndex, std::span<const uint32_t> values,
			uint32_t destOffset = 0) const noexcept;
		template <typename T>
		void SetGraphicsRoot32BitConstants(
			uint32_t parameterIndex, const T& values, uint32_t destOffset = 0) const noexcept
		{
			...
			SetGraphicsRoot32BitConstants(
				parameterIndex, std::span<const uint32_t>(data), destOffset);
		}
		...
		void ClearDepthStencil(const DX12DescriptorView& dsDescriptor, float depthClearValue,
			std::optional<uint8_t> stencilClearValue = std::nullopt) const noexcept;
