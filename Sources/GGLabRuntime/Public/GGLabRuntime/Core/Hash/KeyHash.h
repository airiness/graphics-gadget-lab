...
namespace gglab
{
	template <class T>
	concept Fnv1aScalar =
		std::integral<std::remove_cvref_t<T>> || std::is_enum_v<std::remove_cvref_t<T>> ||
		std::floating_point<std::remove_cvref_t<T>> || std::is_pointer_v<std::remove_cvref_t<T>> ||
	...
	struct FNV1a64 final
	{
		...
		static constexpr void MixValue(uint64_t& hash, const T& value) noexcept
		{
			...
			else if constexpr (std::is_integral_v<U>)
			{
				...
				using UnsignedU =
					std::conditional_t<std::is_same_v<U, bool>, uint8_t, std::make_unsigned_t<U>>;
			...
			{
				static_assert(
					sizeof(U) <= sizeof(uint64_t), "Floating point type is too large to hash.");
				if constexpr (sizeof(U) == sizeof(float))
				{
					constexpr auto canonicalZero = [](float v) noexcept
				...
				{
					constexpr auto canonicalZero = [](double v) noexcept
						{ return v == 0.0 ? 0.0 : v; };
		...
		{
			std::apply([&hash](auto const&... vals) { (MixValue(hash, vals), ...); }, values);
		}
		...
		{
			uint64_t hash = OffsetBasis;
			std::apply([&hash](auto const&... values) { (MixValue(hash, values), ...); }, tuple);
			return static_cast<size_t>(hash);
		}

		[[nodiscard]] static constexpr uint64_t HashBytes64(
			const void* data, size_t length, uint64_t seed = OffsetBasis) noexcept
		{
			uint64_t hash = seed;
