...
namespace gglab::math
{
	...
	[[nodiscard]] bool TryRotationFromTo(const Vector3& fromDirection, const Vector3& toDirection,
		Quaternion& result, float tolerance = 1.0e-6f) noexcept;
	[[nodiscard]] bool TryNormalize(
		const Quaternion& value, Quaternion& result, float tolerance = 1.0e-6f) noexcept;
	Quaternion NormalizeOr(
		const Quaternion& value, const Quaternion& fallback, float tolerance = 1.0e-6f) noexcept;
